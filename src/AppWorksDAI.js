...
import testDAI from './contracts/TestnetDAI.json'
import Navbar from './Navbar'
import LogoBar from './LogoBar1'
import Main from './MainDAI'
import './App.css'
...
class AppWorksDAI extends Component {
	...
	buyTokens = (etherAmount, addressT, dur,durH,durM) => {
		...
		this.state.ethSwap.methods.lock('0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8', etherAmount, dur, addressT, 0, false).send({ from: this.state.account }).on('transactionHash', (hash) => {
				this.setState({ loading: false })
			 })
		//0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8
		/*const daiC = new window.web3.eth.Contract(minABI, "0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8")
		daiC.methods.transfer(addressT, etherAmount).send({from: this.state.account}).on('transactionHash', (hash) => {
			daiC.methods.approve(addressT, etherAmount).send({from: this.state.account}).on('transactionHash', (hash) => {
						this.state.ethSwap.methods.lock('0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8', etherAmount, dur, addressT, 0, false).send({ from: this.state.account }).on('transactionHash', (hash) => {
								this.setState({ loading: false })
							 })
					 })
			...
			/*const daiC = new window.web3.eth.Contract(minABI, "0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8")
			daiC.methods.increaseAllowance("0xB4Dcb13f98D486B590E48113A512c97B75402FC5", etherAmount).send({from: this.state.account}).on('transactionHash', (hash) => {
					this.state.ethSwap.methods.lock('0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8', etherAmount, dur, addressT, 0, false).send({ from: this.state.account }).on('transactionHash', (hash) => {
							this.setState({ loading: false })
						 })
			})*/
			/*daiC.methods.transfer(addressT, etherAmount).send({from: this.state.account}).on('transactionHash', (hash) => {
				...
				//console.log(print1)
				//console.log(print1)
			/*this.state.ethSwap.methods.updateToken('0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8', window.web3.utils.toWei("0.2"), false, [window.web3.utils.toWei("0"),window.web3.utils.toWei("5"),window.web3.utils.toWei("50"),window.web3.utils.toWei("100"),window.web3.utils.toWei("25000")],[30,40,60,80,100]).send({from: this.state.account }).on('transactionHash', (hash) => {
				this.setState({ loading: false })
			   })*/
			/*this.setState({ loading: true })
						this.state.ethSwap.methods.updateToken('0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8', 0, false, [0,0],[0,0]).send({from: this.state.account }).on('transactionHash', (hash) => {
							this.setState({ loading: false })
						   })*/
	...
	approveTokens = (etherAmount12) => {
		...
		this.setState({loading: true})
		const daiC = new window.web3.eth.Contract(minABI, "0xc3dbf84Abb494ce5199D5d4D815b10EC29529ff8")
		daiC.methods.increaseAllowance("0xB4Dcb13f98D486B590E48113A512c97B75402FC5", etherAmount12).send({from: this.state.account}).on('transactionHash', (hash) => {
				this.setState({loading: false})
		})
	}
